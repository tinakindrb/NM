...
@testset "Bézierjeva krivulja" begin
    ...
    t = [0.0, 0.5, 1.0]
    rezultat = bezier(kontrolne_tocke, t)
    ...
    krivulja = bezier(kontrolne_tocke2, [0.0, 1.0])
    @test krivulja[1,:] == kontrolne_tocke2[1,:]
    @test krivulja[end,:] == kontrolne_tocke2[end,:]
end

@testset "Bézierjeva krivulja z odvodom" begin
    # Test 1: linearna Bézierjeva krivulja med (0,0) in (1,1) -> premica y=x
    kontrolne_tocke = [0.0 0.0; 1.0 1.0]
    t = [0.0, 0.5, 1.0]
    krivulja, odvodi = bezier_in_odvod(kontrolne_tocke, t)
    ...
    # Test 2: kvadratna Bézierjeva krivulja s simetrijo
    kontrolne_tocke2 = [0.0 0.0; 1.0 2.0; 2.0 0.0]
    t2 = [0.0, 0.5, 1.0]
    krivulja2, odvodi2 = bezier_in_odvod(kontrolne_tocke2, t2)
...
@testset "Ploščina Bézierjeve krivulje" begin
    ...
    A = ploscina_bezier(kontrolne_tocke; n=1000)
    @test isfinite(A)

    # Lastnost: če krivuljo obrneš (zamenjaš vrstni red točk),
    # se ploščina spremeni za predznak
    kontrolne_tocke_obrnjene = reverse(kontrolne_tocke, dims=1)
    A2 = ploscina_bezier(kontrolne_tocke_obrnjene; n=1000)
    ...
    A3 = ploscina_bezier(kontrolne_tocke_premica; n=1000)
    @test isapprox(A3, 0.0; atol=1e-6)

    # Primer : kompleksnejši kontrolni poligon
    kontrolne_tocke2 = [
        0.0 0.0;
        1.0 1.0;
        2.0 3.0;
    ...
    A2 = ploscina_bezier(kontrolne_tocke2; n=2000)
    @test isapprox(A2, 1.966200466200; atol=1e-5)
end
